...
### Example 2: Capturing caller arguments

You can skip `args` entirely and let `log_add()` capture all objects in the calling environment:

```{r}
process_data <- function(file, limit = 100, ...) {
  log_add("info", "process 1", name = "mylog")
  log_add("info", "process 2", name = "mylog", .env = environment())
  # do something
}
process_data("input.csv", limit = 10)
l <- log_get("mylog")
```

The log will automatically store:
```r
$args = list(file = "input.csv", limit = 10)
```

If `...` contains other arguments, those are captured too!
