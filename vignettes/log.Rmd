...
With the new pins-based interface, you save and reload logs using a pins board and a pin name:

```{r}
# Create a temporary pins board (in practice, use your persistent board)
board <- pins::board_temp(versioned = TRUE)
pin_name <- "pipapi_log_pin"

# Save the log to the board
log_save(name = "pipapi_log", board = board, pin_name = pin_name)

# Remove from memory
log_reset("pipapi_log")

# Load the log back from the board
log_load(board = board, pin_name = pin_name, overwrite = TRUE)
```
